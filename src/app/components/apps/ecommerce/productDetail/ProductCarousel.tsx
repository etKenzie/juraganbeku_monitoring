...
const ProductCarousel = () => {
  ...
  // Get Products
  const product: ProductType = useSelector((state) => state.ecommerceReducer.products[getTitle - 1]);
  ...
  return (
    <Box>
      <Slider asNavFor={nav2} ref={(slider: any) => (slider1.current = slider)}>
        <Box>
          <Image src={getProductImage} alt={getProductImage} width={500} height={500} style={{ borderRadius: "5px", width: "100%", height: "auto" }} />
        </Box>
        {SliderData.map((step) => (
          <Box key={step.id}>
            <Image src={step.imgPath} width={500} height={500} alt={step.imgPath} style={{ borderRadius: "5px", width: "100%", height: "auto" }} />
          </Box>
        ))}
      </Slider>
      <Slider asNavFor={nav1} ref={(slider: any) => (slider2.current = slider)} {...settings}>
        <Box sx={{ p: 1, cursor: "pointer" }}>
          <Image src={getProductImage} alt={getProductImage} width={72} height={72} style={{ borderRadius: "5px" }} />
        </Box>
        {SliderData.map((step) => (
          <Box key={step.id} sx={{ p: 1, cursor: "pointer" }}>
            <Image src={step.imgPath} alt={step.imgPath} width={72} height={72} style={{ borderRadius: "5px" }} />
          </Box>
        ))}
      </Slider>
    </Box>
